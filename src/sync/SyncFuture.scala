...
/** The trait that represents 'synchronous' futures. Taken from the asynchronous version built into scala.
  *
  *
...
trait SyncFuture[+T] {
  ...
  /** Returns a failed projection of this future.
    ...
    */
  def failed: SyncFuture[Throwable] = {
    val p = SyncPromise[Throwable]()
  ...
  /** Creates a new future by applying the 's' function to the successful result of
    ...
    */
  def transform[S](s: T => S, f: Throwable => Throwable): SyncFuture[S] = {
    val p = SyncPromise[S]()
  ...
  /** Creates a new future by applying a function to the successful result of
    ...
    * $forComprehensionExamples
    */
  def flatMap[S](f: T => SyncFuture[S]): SyncFuture[S] = {
    val p = SyncPromise[S]()
  ...
  /** Creates a new future by filtering the value of the current future with a predicate.
    ...
    * }}}
    */
  def filter(pred: T => Boolean): SyncFuture[T] = {
    val p = SyncPromise[T]()
  ...
  /** Used by for-comprehensions.
    */
  final def withFilter(p: T => Boolean): SyncFuture[T] = filter(p)
  ...
  /** Creates a new future by mapping the value of the current future, if the given partial function is defined at that value.
    ...
    * }}}
    */
  def collect[S](pf: PartialFunction[T, S]): SyncFuture[S] = {
    val p = SyncPromise[S]()
  ...
  /** Creates a new future that will handle any matching throwable that this
    ...
    * }}}
    */
  def recover[U >: T](pf: PartialFunction[Throwable, U]): SyncFuture[U] = {
    val p = SyncPromise[U]()
  ...
  /** Creates a new future that will handle any matching throwable that this
    ...
    * }}}
    */
  def recoverWith[U >: T](pf: PartialFunction[Throwable, SyncFuture[U]]): SyncFuture[U] = {
    val p = SyncPromise[U]()
  ...
  /** Zips the values of `this` and `that` future, and creates
    ...
    */
  def zip[U](that: SyncFuture[U]): SyncFuture[(T, U)] = {
    val p = SyncPromise[(T, U)]()
  ...
  /** Creates a new future which holds the result of this future if it was completed successfully, or, if not,
    ...
    * }}}
    */
  def fallbackTo[U >: T](that: SyncFuture[U]): SyncFuture[U] = {
    val p = SyncPromise[U]()
    onComplete {
  ...
  /** Creates a new `Future[S]` which is completed with this `Future`'s result if
    ...
    */
  def mapTo[S](implicit tag: ClassTag[S]): SyncFuture[S] = {
    def boxedType(c: Class[_]): Class[_] = {
      if (c.isPrimitive) SyncFuture.toBoxed(c) else c
    }
  ...
  /** Applies the side-effecting function to the result of this future, and returns
    ...
    * }}}
    */
  def andThen[U](pf: PartialFunction[Try[T], U]): SyncFuture[T] = {
    val p = SyncPromise[T]()
...
object SyncFuture {
  ...
  def apply[T](body: =>T): SyncFuture[T] = {
  ...
  /** Creates an already completed Future with the specified exception.
    ...
    */
  def failed[T](exception: Throwable): SyncFuture[T] = SyncPromise.failed(exception).future
  ...
  /** Creates an already completed Future with the specified result.
    ...
    * @return         the newly created `Future` object
    */
  def successful[T](result: T): SyncFuture[T] = SyncPromise.successful(result).future

  /** Simple version of `Futures.traverse`. Transforms a `TraversableOnce[Future[A]]` into a `Future[TraversableOnce[A]]`.
    * Useful for reducing many `Future`s into a single `Future`.
    */
  def sequence[A, M[_] <: TraversableOnce[_]](in: M[SyncFuture[A]])(implicit cbf: CanBuildFrom[M[SyncFuture[A]], A, M[A]]): SyncFuture[M[A]] = {
    in.foldLeft(SyncPromise.successful(cbf(in)).future) {
      (fr, fa) => for (r <- fr; a <- fa.asInstanceOf[SyncFuture[A]]) yield (r += a)
    } map (_.result)
  }

  /** Returns a `Future` to the result of the first future in the list that is completed.
    */
  def firstCompletedOf[T](futures: TraversableOnce[SyncFuture[T]]): SyncFuture[T] = {
    val p = SyncPromise[T]()
  ...
  /** Returns a `Future` that will hold the optional result of the first `Future` with a result that matches the predicate.
    */
  def find[T](futurestravonce: TraversableOnce[SyncFuture[T]])(predicate: T => Boolean): SyncFuture[Option[T]] = {
    val futures = futurestravonce.toBuffer
    if (futures.isEmpty) SyncPromise.successful[Option[T]](None).future
    else {
      val result = SyncPromise[Option[T]]()
  ...
  /** A non-blocking fold over the specified futures, with the start value of the given zero.
    ...
    * }}}
    */
  def fold[T, R](futures: TraversableOnce[SyncFuture[T]])(zero: R)(foldFun: (R, T) => R): SyncFuture[R] = {
    if (futures.isEmpty) SyncPromise.successful(zero).future
  ...
  /** Initiates a fold over the supplied futures where the fold-zero is the result value of the `Future` that's completed first.
    ...
    * }}}
    */
  def reduce[T, R >: T](futures: TraversableOnce[SyncFuture[T]])(op: (R, T) => R): SyncFuture[R] = {
    if (futures.isEmpty) SyncPromise[R].failure(new NoSuchElementException("reduce attempted on empty collection")).future
  ...
  /** Transforms a `TraversableOnce[A]` into a `Future[TraversableOnce[B]]` using the provided function `A => Future[B]`.
    ...
    * }}}
    */
  def traverse[A, B, M[_] <: TraversableOnce[_]](in: M[A])(fn: A => SyncFuture[B])(implicit cbf: CanBuildFrom[M[A], B, M[B]]): SyncFuture[M[B]] =
    in.foldLeft(SyncPromise.successful(cbf(in)).future) { (fr, a) =>
